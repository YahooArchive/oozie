...
import org.apache.oozie.command.jpa.WorkflowsJobGetCommand;
...
public class JobsXCommand extends WorkflowXCommand<WorkflowsInfo> {
    ...
    protected WorkflowsInfo execute() throws CommandException {
        try {
            ...
            if (jpaService != null) {
                this.workflows = jpaService.execute(new WorkflowsJobGetCommand(this.filter, this.start, this.len));
            }
            else {
