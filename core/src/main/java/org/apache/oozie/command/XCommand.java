...
public abstract class XCommand<T> implements XCallable<T> {
	 ...
	 * <p/>
	 * The command type is used for concurrency throttling in the
	 * {@link CallableQueueService}.
	 * 
	...
	/**
	 * Queue a command for execution after the current command execution
	 * completes.
	 * <p/>
	 * All commands queued during the execution of the current command will be
	 * queued for a single serial execution.
	 * <p/>
	 * If the command execution throws an exception, no command will be
	 * effectively queued.
	 * 
	 * @param command
	 *            command to queue.
	...
	/**
	 * Queue a command for delayed execution after the current command execution
	 * completes.
	 * <p/>
	 * All commands queued during the execution of the current command with the
	 * same delay will be queued for a single serial execution.
	 * <p/>
	 * If the command execution throws an exception, no command will be
	 * effectively queued.
	 * 
	 ...
	 * <p/>
	 * A timeout of {link #getLockTimeOut} is used when trying to obtain the
	 * lock.
	 * 
	 * @throws InterruptedException
	 *             thrown if an interruption happened while trying to obtain the
	 *             lock
	 * @throws CommandException
	 *             thrown i the lock could not be obtained.
	 */
	private void acquireLock() throws InterruptedException, CommandException {
		lock = Services.get().get(MemoryLocksService.class).getWriteLock(
				getEntityKey(), getLockTimeOut());
		if (lock == null) {
			Instrumentation instrumentation = Services.get().get(
					InstrumentationService.class).get();
	 ...
	 * @throws Exception
	 *             thrown if the command could not be executed.
	 */
	@Override
	public final T call() throws Exception {
		...
		try {
			...
			try {
				if (isLockRequired()) {
					...
					try {
						...
						instrumentation.addCron(INSTRUMENTATION_GROUP,
								getName() + ".acquireLock", acquireLockCron);
					}
				}
				getLog().debug("Load state for [{0}]", getEntityKey());
				loadState();
				getLog().debug(
						"Precondition check for command [{0}] key [{1}]",
						getName(), getEntityKey());
				verifyPrecondition();
				getLog().debug("Execute command [{0}] key [{1}]", getName(),
						getEntityKey());
				...
				if (commandQueue != null) {
					CallableQueueService callableQueueService = Services.get()
							.get(CallableQueueService.class);
					for (Map.Entry<Long, List<XCommand<T>>> entry : commandQueue
							.entrySet()) {
						getLog().debug(
								"Queuing [{0}] commands with delay [{1}]ms",
								entry.getValue().size(), entry.getKey());
						if (!callableQueueService.queueSerial(entry.getValue(),
								entry.getKey())) {
							getLog()
									.warn(
											"Could not queue [{0}] commands with delay [{1}]ms, queue full",
											entry.getValue().size(),
											entry.getKey());
						}
					}
			...
			callCron.stop();
			instrumentation.addCron(INSTRUMENTATION_GROUP, getName() + ".call",
					callCron);
		}
	}
	 ...
	 * <p/>
	 * Subclasses should override this method if the want to use a different log
	 * instance.
	 * 
	 ...
	 * <p/>
	 * The value is loaded from the Oozie configuration, the property {link
	 * #DEFAULT_LOCK_TIMEOUT}.
	 * <p/>
	 * Subclasses should override this method if they want to use a different
	 * time out.
	 * 
	 ...
	 * <p/>
	 * Subclasses should override this method and load the state needed to do an
	 * eager precondition check.
	 * <p/>
	...
	/**
	 * Load the necessary state to perform the precondition check and to execute
	 * the command.
	 * <p/>
	 * Subclasses must implement this method and load the state needed to do the
	 * precondition check and execute the command.
	 */
	protected abstract void loadState();

	/**
	 * Verify the precondition for the command after a lock has been obtain,
	 ...
	 * <p/>
	 * This method will be invoked after the {link #loadState} and {link
	 * #verifyPrecondition} methods.
	 * <p/>
	 * If the command requires locking, this method will be invoked ONLY if the
	 * lock has been acquired.
	 * 
	 * @return a return value from the execution of the command, only meaningful
	 *         if the command is executed synchronously.
	 * @throws CommandException
